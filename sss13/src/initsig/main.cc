...
#include <initsig/child.h>
#include <base/sleep.h>
#include <os/config.h>

/* import public key */
#include <initsig/publickey.h>

namespace InitSig { bool config_verbose = false; }
	...
	for (;;) {
		...
		try {
			...
			for (;; start_node = start_node.next("start")) {
				...
				try {
					InitSig::Child *child = new (env()->heap()) InitSig::Child(start_node, default_route_node,
				...
				catch (InitSig::Child::Signature_failed) {
		...
		catch (InitSig::Child::Child_name_is_not_unique) { }
		...
		while (children.any()) {
			InitSig::Child *child = children.any();
