...
#ifndef _INCLUDE__INITSIG__CHILD_H_
#define _INCLUDE__INITSIG__CHILD_H_
...
/* init includes */
#include <initsig/child_config.h>
#include <initsig/child_policy.h>
...
namespace InitSig {
	...
	class Child : Genode::Child_policy
	{
		...
		private:
			 ...
			 * Policy helpers
			 */
			InitSig::Child_policy_enforce_labeling      _labeling_policy;
			InitSig::Child_policy_handle_cpu_priorities _priority_policy;
			InitSig::Child_policy_provide_rom_file      _config_policy;
			InitSig::Child_policy_provide_rom_file      _binary_policy;
			InitSig::Child_policy_redirect_rom_file     _configfile_policy;
			InitSig::Child_policy_pd_args               _pd_args_policy;
		...
		public:
				...
				_pd_args_policy(&_pd_args)
			{
				...
				/*
				 * Get signature value from config
				 */
				char value[256] = {0};
				try {
					Xml_node sign_node = start_node.sub_node("signature");
					sign_node.attribute("value").value(value, sizeof(value));
					if (config_verbose)
						Genode::printf("config signature value=%s\n", value);
				} catch (Xml_node::Nonexistent_sub_node) {
					throw;
				}
				
				if (Genode::strlen(value) != NUM_ECC_DIGITS * 4 * sizeof(uint32_t)) {
					PERR("%s: wrong signature value", _name.unique);
					throw Signature_failed();
				}
				
				/* TODO: convert signature string to bibary */
				uint32_t sign[NUM_ECC_DIGITS*2];
				sign2bn(value, sign);
				
				/* TODO: get binary address and size */
				addr_t _addr;
				size_t _size;
				
				/* TODO: calculate hash */
				unsigned char hash[SHA256_DIGEST_LENGTH];
				calculate_hash(_addr, _size, hash);
				
				/* verify public key */
				if(!ecc_valid_public_key(&k_public)) {
					PERR("%s: Not a valid public key!", _name.unique);
					throw Signature_failed();
				}
				
				/* TODO: verify signature */
				
				PINF("%s: Signature correct", _name.unique);
				
				/* create child */
				_child = new (Genode::env()->heap()) Genode::Child(_binary_rom.dataspace(), _resources.ram.cap(),
...
#endif /* _INCLUDE__INITSIG__CHILD_H_ */
